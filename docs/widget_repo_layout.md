...
1. The last thing you do before committing is run `rhtml build` to ensure all the autogenerated files are up to date.
1. (With some exceptions) **ONLY EDIT THINGS IN these directories: `theSrc`, `docs`, `build`, and `bdd` !!** Many of the other files are auto generated based on the contents of `theSrc`. As an example, if you edit `R/rhtmlTemplate.R` and then run `rhtml build` your changes will be DELETED FOREVER!, because `R/rhtmlTemplate.R` is just a copy of `theSrc/R/htmlwidget.R`.
...
Given we use npm and package.json to manage our dependencies, and given we have a browser/ directory that supports the `rhtml serve` workflow, maintaining our code in the structure outlined above is not really a good option. Instead, we choose to maintain all the source code in a directory called `theSrc` (if it was called `src` then the HTMLWidget package will treat it as binaries ??) and use a build process to automatically generate the R htmlwidget file structure above from the source in `theSrc`.
...
_**So lets recap a really important point: only edit files in the `theSrc`, `docs`, `build`' and `bdd` directories.**_. All files in the `R/`, `inst/`, and `man/` directories are autogenerated by the `rhtml build` / `rhtml serve` task using input from the `theSrc` directory. If you edit in `inst` and then run `rhtml build` it will wipe out all your hard work, and your peers will laugh at your misfortune !
...
* **gulpfile.js** - REMOVED in rhtmlBuildUtils 9.0.0. Tasks are run by the `rhtml` binary and are discovered automatically, so there is no registration file. A repo that defines its own gulp tasks may keep a gulpfile for those alone.
...
* **browser/** - this content is **autogenerated** and is used to test and develop the htmlwidget in a chrome browser context - this is done when running rhtml serve.
...
**HTML Widget Packages**: A R HTML Widget is a special type of R package that contains R code, as well as Javascript. In the case of most HTML Widgets (warning arbitrary stat) 95%  of the code is Javascript and there is very little R code. The majority of our 3rd party dependencies are Javascript packages, so it makes sense to adopt a dependency management system and best practices from the nodejs community (i.e., npm packaging and a node based build tool). However, the end product is still R code. We need a way to use npm to dynamically pull all our dependencies, but still produce a compliant R HTMLWidget package, enter the ....

**Build System**: rhtmlBuildUtils ships a task runner, invoked as `rhtml <task>`, providing standard ways to define multi step development tasks such as code packaging, code testing and code compiling. It's kind of like `make`. This replaced gulp in 9.0.0, and is easiest understood by providing examples, which are provided in the next section.
